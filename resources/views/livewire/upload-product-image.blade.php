<div class="w-full flex flex-wrap justify-between gap-4">
    @foreach ($images as $index => $image)
        <label
            ...
            <input wire:model="images.{{ $index }}" name="images[{{ $index }}]" type="file" accept="image/*" class="hidden">
            @if ($image)
                <img src="{{ $image->temporaryUrl() }}" class="w-full h-full object-contain">
                <button class="group-hover:flex hidden absolute bottom-0 right-0 text-gray-500 p-2 rounded-full">
