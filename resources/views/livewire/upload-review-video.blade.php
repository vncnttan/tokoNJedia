<div class="w-full flex flex-wrap justify-between gap-4">
    @foreach ($videos as $index => $video)
        <label
            ...
            @if ($video)
                <video src="{{ $video->temporaryUrl() }}"
                       class="w-full h-full object-contain z-0" muted> </video>
                <button type="button" class="group-hover:flex z-20 hidden absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white p-2 w-full h-full bg-black bg-opacity-25 justify-center items-center"
